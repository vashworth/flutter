...
Future<void> main(List<String> args) async {
  ...
  await task(() async {
    ...
    final String? scenarioTarget = argResults.option(kScenario);
    ...
    try {
      ...
      await testWithNewIOSSimulator('TestAdd2AppSim', (String deviceId) async {
        ...
        for (final String scenarioName in scenarios.scenarios.keys) {
          if (scenarioTarget != null && scenarioName != scenarioTarget) {
            continue;
          }
          ...
          // Reset files between scenarios with defaults
...
class FileReplacements {
  ...
  void replace() {
    ...
    if (!destinationFile.existsSync()) {
      ...
      _originalContent = destinationFile.readAsStringSync();
    }
  ...
  void reset() {
    ...
    if (_originalContent != null) {
      destinationFile.writeAsStringSync(_originalContent!);
    }
  }
}
...
class Scenarios {
  ...
  late Map<String, List<FileReplacements>> scenarios = <String, List<FileReplacements>>{
    ...
    // When using FlutterImplicitEngineInitializationListener, we expect plugins to be registered after the
    // FlutterViewController is created, which results in beginning application events being lost.
    'FlutterImplicitEngineInitializationListener-AppMigrated-ImplicitFlutterEngine':
        <FileReplacements>[
          ...
          FileReplacements(
            path.join(
              templatesDir.path,
              'native',
              'AppDelegate-FlutterImplicitEngineInitializationListener.swift',
            ),
            path.join(xcodeProjectDir.path, 'xcode_uikit_swift', 'AppDelegate.swift'),
          ),
          FileReplacements(
            path.join(templatesDir.path, 'native', 'ViewController-ImplicitFlutterEngine.swift'),
          ...
          FileReplacements(
            path.join(
              templatesDir.path,
              'native',
              'UITests-SceneEvents-FlutterImplicitEngineInitializationListener.swift',
            ),
            path.join(
              xcodeProjectDir.path,
              'xcode_uikit_swiftUITests',
    ...
    // When using FlutterImplicitEngineInitializationListener, we expect plugins to be registered after the
    // FlutterViewController is created, which results in beginning application events being lost.
    'FlutterImplicitEngineInitializationListener-AppMigrated-StoryboardFlutterViewController':
        <FileReplacements>[
          ...sharedAppLifecycleFiles,
          ...sharedPluginLifecycleFiles,
          FileReplacements(
          ...
          FileReplacements(
            path.join(templatesDir.path, 'native', 'Main-FlutterViewController.storyboard'),
            path.join(xcodeProjectDir.path, 'xcode_uikit_swift', 'Base.lproj', 'Main.storyboard'),
          ),
          FileReplacements(
            path.join(
              templatesDir.path,
    ...
    // When using FlutterImplicitEngineInitializationListener, we expect plugins to be registered after the
    // FlutterViewController is created, which results in beginning application events being lost.
    'FlutterImplicitEngineInitializationListener-AppNotMigrated-ImplicitFlutterEngine':
        <FileReplacements>[
          ...sharedAppLifecycleFiles,
          ...sharedPluginLifecycleFiles,
          FileReplacements(
          ...
          FileReplacements(
            path.join(
              templatesDir.path,
              'native',
              'UITests-ApplicationEvents-FlutterImplicitEngineInitializationListener.swift',
            ),
            path.join(
              xcodeProjectDir.path,
              'xcode_uikit_swiftUITests',
    ...
    // When using FlutterImplicitEngineInitializationListener, we expect plugins to be registered after the
    // FlutterViewController is created, which results in beginning application events being lost.
    'FlutterImplicitEngineInitializationListener-AppNotMigrated-StoryboardFlutterViewController':
        <FileReplacements>[
          ...sharedAppLifecycleFiles,
          ...sharedPluginLifecycleFiles,
          FileReplacements(
          ...
          FileReplacements(
            path.join(
              templatesDir.path,
              'native',
              'UITests-ApplicationEvents-FlutterImplicitEngineInitializationListener-Storyboard.swift',
            ),
            path.join(
              xcodeProjectDir.path,
              'xcode_uikit_swiftUITests',
  ...
  late List<FileReplacements> sharedLifecycleFiles = <FileReplacements>[
    ...sharedAppLifecycleFiles,
    ...sharedPluginLifecycleFiles,
    ...sharedNativeLifecycleFiles,
  ];

  late List<FileReplacements> sharedAppLifecycleFiles = <FileReplacements>[
    FileReplacements(
  ...
  late List<FileReplacements> sharedPluginLifecycleFiles = <FileReplacements>[
    FileReplacements(
  ...
  late List<FileReplacements> sharedNativeLifecycleFiles = <FileReplacements>[
    FileReplacements(
