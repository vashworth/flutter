...
class SwiftPackageManager {
  ...
  // packageDirectory = Packages | .packages
  // flutterPluginSwiftPackageDirectory = FlutterPluginRegistrant | .packages/FlutterGeneratedPluginSwiftPackage
  Future<
    (
  ...
  _createPluginDependencies({
    ...
    final (
      ...
      plugins: plugins,
      platforms: platforms,
      symlinkDirectory: buildModeDirectory,
      pathRelativeTo: flutterPluginSwiftPackageDirectory.path,
      defaultBuildMode: defaultBuildMode,
    );
  ...
  /// Create a sub-directory for each BuildMode in [packageDirectory].
  /// Then for each platform, for each plugin that supports SwiftPM, symlink it in the sub-directory.
  ...
  /// Symlinks the Flutter/FlutterMacOS framework xcframeworks into [packageDirectory] if required and create a Swift Package in [packageDirectory].
  ///
  /// If [remoteFramework] is true, use a url to the zip in the Swift Package instead of a symlinked framework.
  ///
  /// If [frameworkSource] is provided, symlink from [frameworkSource] instead of from the engine cache.
  ...
  _dependenciesForPlugins({
    ...
    // TODO: SPM - handle when package supports both iOS and macOS?
    for (final Plugin plugin in plugins) {
      for (final DarwinPlatform platform in platforms) {
        final bool supportsSwiftPackageManager = plugin.supportsSwiftPackageManager(
          _fileSystem,
