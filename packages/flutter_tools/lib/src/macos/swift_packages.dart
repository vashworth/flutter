...
class SwiftPackage {
  ...
  Future<void> convertPodToSwiftPackage(Plugin plugin) async {
    final Status progress = _logger.startProgress('Converting ${plugin.name} to a Swift package...');
    if (overwriteExisting == false && await swiftPackage.exists()) {
      _logger.printError('A Package.swift was already found');
      return;
    }
    final Directory pluginIOSDirectory = _fileSystem.directory(_fileSystem.path.join('${plugin.path}ios'));
    if (!pluginIOSDirectory.existsSync()) {
      _logger.printStatus('Unable to find ios directory');
      return;
    }

    final PodspecMigration podspecMigration = PodspecMigration(
      podspecPath: pluginIOSDirectory.childFile('${plugin.name}.podspec').path,
      fileSystem: _fileSystem,
      logger: _logger,
    );

    await podspecMigration.parsePodspec();

    String? sourceFilesPath;
    String? publicHeaderFilesPath;

    Directory? classesDirectory;

    if (podspecMigration.rootSpecifications.containsKey(PodspecSpecification.sourceFiles)) {
      final List<String> sourceFiles = podspecMigration.rootSpecifications[PodspecSpecification.sourceFiles] as List<String>;
      if (sourceFiles.isNotEmpty && sourceFiles.first.startsWith('Classes/**/*')) {
        classesDirectory = pluginIOSDirectory.childDirectory('Classes');

        sourceFilesPath = './Classes';
      }
    }
    if (podspecMigration.rootSpecifications.containsKey(PodspecSpecification.publicHeaderFiles)) {
      final List<String> publicHeaderFiles = podspecMigration.rootSpecifications[PodspecSpecification.publicHeaderFiles] as List<String>;
      if (publicHeaderFiles.isNotEmpty && publicHeaderFiles.first == 'Classes/**/*.h') {
        publicHeaderFilesPath = '.';
      }
    } else if (sourceFilesPath != null) {
      publicHeaderFilesPath = '.';
    }

    if (podspecMigration.rootSpecifications.containsKey(PodspecSpecification.moduleMap)) {
      final String moduleMapPath = podspecMigration.rootSpecifications[PodspecSpecification.moduleMap] as String;
      final File moduleMapFile = _fileSystem.file('${pluginIOSDirectory.path}/$moduleMapPath');
      final List<String> lines = await moduleMapFile.readAsLines();
      final List<String> newLines = <String>[];
      for (String line in lines) {
        String newLine = line;
        if (line.startsWith('framework module')) {
          newLine = line.replaceFirst('framework module', 'module');
        }
        if (line.trim().contains('header')) {
          List<String> parts = line.split('"');
          newLine = '${parts[0]}"${plugin.name}/${parts[1]}"';
        }
    ...
    final SwiftPackageContext packageContext = SwiftPackageContext(
      ...
      products: <SwiftPackageProduct>[
        SwiftPackageProduct(name: plugin.name, targets: <String>[plugin.name]),
      ],
      ...
      targets: <SwiftPackageTarget>[
        SwiftPackageTarget(
          ...
          publicHeadersPath: publicHeaderFilesPath,
        ),
      ],
    );
    ...
    // globals.localFileSystem.file('${plugin.path}ios/${plugin.name}/Package.swift').existsSync()
  }
