...
- (void)testNotifyAppDelegateOfEngineInitialization {
  ...
  id mockAppDelegate = OCMProtocolMock(@protocol(FlutterImplicitEngineInitializationListener));
  OCMStub([mockApplication delegate]).andReturn(mockAppDelegate);

  [engine notifyAppDelegateOfEngineInitialization];
  OCMVerify(times(1), [mockAppDelegate flutterImplicitEngineDidInitialize:[OCMArg any]]);
}
