...
@property(nonatomic, readonly)
    NSMutableDictionary<NSString*, NSObject<FlutterBaseRegistrar>*>* registrars;
...
@implementation FlutterEngineInterface {
}
...
- (instancetype)initWithName:(NSString*)labelPrefix
  ...
  self.engineInterface = [[FlutterEngineInterface alloc] initWithEngine:self];
...
- (void)dealloc {
  ...
  [_pluginPublications enumerateKeysAndObjectsUsingBlock:^(id key, id object, BOOL* stop) {
    if ([object respondsToSelector:@selector(detachFromEngineForRegistrar:)]) {
      NSObject<FlutterBaseRegistrar>* registrar = self.registrars[key];
  ...
  [_registrars enumerateKeysAndObjectsUsingBlock:^(
                   id key, NSObject<FlutterBaseRegistrar>* registrar, BOOL* stop) {
    if ([registrar isKindOfClass:[FlutterEnginePluginRegistrar class]]) {
      ((FlutterEnginePluginRegistrar*)registrar).flutterEngine = nil;
    }
  }];
...
- (void)launchEngine:(NSString*)entrypoint
  ...
  // Callback to register plugins
  id appDelegate = FlutterSharedApplication.application.delegate;
  if ([appDelegate conformsToProtocol:@protocol(FlutterEngineProvider)]) {
    id<FlutterEngineProvider> provider = (id<FlutterEngineProvider>)appDelegate;
    [provider connectWithEngineInterface:self.engineInterface];
  }
}
...
- (void)addApplicationDelegate:(NSObject<FlutterPlugin>*)delegate {
  // FML_LOG(ERROR) << "Registering plugins as delegates";
  id appDelegate = FlutterSharedApplication.application.delegate;
  ...
  if ([delegate conformsToProtocol:@protocol(FlutterSceneLifeCycleDelegate)]) {
    ...
    [self.flutterEngine addSceneLifeCycleDelegate:sceneDelegate];
  }
}

- (UIView*)view {
  return self.flutterEngine.viewController.view;
